...
FROM_EMAIL = os.getenv('FROM_EMAIL', 'titouan.elmouafik@gmail.com')
...
def send_confirmation_email(reservation):
    """
    Envoie un email de confirmation via l'API Brevo
    """

    if not BREVO_API_KEY:
        print("⚠️  BREVO_API_KEY manquante. Email non envoyé.")
        return False
    ...
    try:
            ...
            "to": [
                {
                    ...
                    "name": f"{reservation['prenom']} {reservation['nom']}"
                }
            ],
        ...
        if response.status_code == 201:
            print(f"✓ Email de confirmation envoyé à {reservation['email']} via API Brevo")
            return True
        else:
...
def send_rejection_email(reservation):
    """
    Envoie un email de refus via l'API Brevo
    """
    ...
    try:
        ...
        if response.status_code == 201:
            print(f"✓ Email de refus envoyé à {reservation['email']} via API Brevo")
            return True
        else:
            print(f"✗ Erreur API Brevo ({response.status_code}): {response.text}")
            return False
